...
for plot_i, y in enumerate([y_age, y_cdr, y_sex]):
    ...
    idxs = []
    columns = ["Cerebral Cortex ", "Cerebral White Matter", "Thalamus", "Amygdala", "Hippocampus", "Pallidum",
...
bps = []
for j in range(3):
    bps.append(plt.boxplot(imp[j].importances[sorted_idx].T, labels=[""] * 10, patch_artist=True,
                vert=True, positions=positions + j))
...
plt.legend([x["boxes"][0] for x in bps], ["Age Prediction", "Alzheimer's Prediction", "Sex Prediction"])
plt.hlines(y=0, xmin=0.5, xmax=len(columns)*4-.5, linestyles="dashed", colors="black")
